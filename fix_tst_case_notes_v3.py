...
def target_value_for(case_kind: str, field_name: str, current_value: str) -> str:
    ...
    if case_kind == CASE_R and field_name == FIELD_SOURCE:
        ...
        # “需求分析”及其仅带句末标点/空白的变体，统一写为规范值。
        if normalized_value == R_SOURCE_REQUIREMENT:
            return R_SOURCE_REQUIREMENT

        # “需求分析、有效等价类”及其带句号、逗号变体，
        # 统一修正为“需求分析、等价类生成与分析”。
        if normalized_value == R_SOURCE_OLD_EQUIVALENCE:
            return R_SOURCE_NEW_EQUIVALENCE

        # 保证脚本幂等：已经修正后的来源即使带句末标点，
        # 再次运行也仍写回对应的规范值，而不会退化成“需求分析”。
        if normalized_value == R_SOURCE_NEW_EQUIVALENCE:
            return R_SOURCE_NEW_EQUIVALENCE
        if normalized_value == R_SOURCE_EQUIVALENCE:

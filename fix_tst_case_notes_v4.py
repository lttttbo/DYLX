...
检查并修正 .tst 文件中指定 TEST.NAME 后缀的测试用例说明：
  ...
  2. .R
     ...
     - 用例来源：
       * “需求分析”保持不变
       * “需求分析、有效等价类”及其顺序颠倒形式，修正为“需求分析、等价类生成与分析”
       * 不包含“需求分析”但包含“有效等价类”时，修正为“等价类生成与分析”
       * 已修正的“需求分析、等价类生成与分析”或“等价类生成与分析”保持不变
       * 其他内容统一修正为“需求分析”
...
# 用例来源经常带有句末标点、全角/半角分隔符或多余空白。
# 这些字符不应改变来源的语义，因此仅在规则判定时做归一化；
# 最终写回文件的仍是上方定义的规范目标值。
SOURCE_TRAILING_PUNCTUATION_RE = re.compile(r"[\s\u3000。．.，,；;：:！？!?、]+$")
SOURCE_SEPARATOR_RE = re.compile(r"[\s\u3000]*[，,；;][\s\u3000]*")
SOURCE_WHITESPACE_RE = re.compile(r"[\s\u3000]+")
SOURCE_WRAPPER_CHARS = "\"'“”‘’「」『』"


def normalize_source_for_matching(value: str) -> str:
    """归一化用例来源，供规则匹配使用，不直接作为写回值。"""
    normalized = value.strip()

    # 兼容：需求分析、有效等价类。
    # 以及：“需求分析、有效等价类。”等常见人工录入格式。
    for _ in range(2):
        normalized = normalized.strip(
            SOURCE_WRAPPER_CHARS + " \t\r\n\u3000"
        )
        normalized = SOURCE_TRAILING_PUNCTUATION_RE.sub("", normalized)

    # 将常见的列表分隔符统一成中文顿号，并去除无意义空白。
    normalized = SOURCE_SEPARATOR_RE.sub("、", normalized)
    normalized = SOURCE_WHITESPACE_RE.sub("", normalized)
    normalized = re.sub(r"、{2,}", "、", normalized).strip("、")
    return normalized


def split_source_items_for_matching(normalized_value: str) -> Tuple[str, ...]:
    """将归一化后的用例来源拆成无顺序语义的来源项。

    例如：
      - 需求分析、有效等价类
      - 有效等价类、需求分析

    两者都会得到相同的来源项集合。这里仍保留元组返回值，便于诊断
    重复项；规则判断时再转换成集合。
    """
    items: List[str] = []
    for raw_item in normalized_value.split("、"):
        item = raw_item.strip(SOURCE_WRAPPER_CHARS + " \t\r\n\u3000")
        if item:
            items.append(item)
    return tuple(items)
...
def target_value_for(case_kind: str, field_name: str, current_value: str) -> str:
    ...
    if case_kind == CASE_R and field_name == FIELD_SOURCE:
        normalized_value = normalize_source_for_matching(current_value)
        source_items = split_source_items_for_matching(normalized_value)
        source_item_set = set(source_items)

        # 先按“来源项集合”判断，而不是比较固定顺序的整串文本。
        # 因此以下两种写法语义完全相同：
        #   需求分析、有效等价类
        #   有效等价类、需求分析
        if source_item_set == {
            R_SOURCE_REQUIREMENT,
            R_SOURCE_EFFECTIVE_EQUIVALENCE,
        }:
            return R_SOURCE_NEW_EQUIVALENCE

        # 保证脚本幂等，并兼容规范值顺序颠倒的人工录入。
        if source_item_set == {
            R_SOURCE_REQUIREMENT,
            R_SOURCE_EQUIVALENCE,
        }:
            return R_SOURCE_NEW_EQUIVALENCE

        if source_item_set == {R_SOURCE_REQUIREMENT}:
            return R_SOURCE_REQUIREMENT
        if source_item_set == {R_SOURCE_EQUIVALENCE}:
            return R_SOURCE_EQUIVALENCE

        # 不包含“需求分析”但包含“有效等价类”时，
        # 无论是否还包含其他来源内容，都统一修正为“等价类生成与分析”。
        if (
            R_SOURCE_REQUIREMENT not in normalized_value
            and R_SOURCE_EFFECTIVE_EQUIVALENCE in normalized_value
        ):
            return R_SOURCE_EQUIVALENCE

        # 含“需求分析”但又混入故障注入等其他非规范来源时，
        # 仍按既定规则统一修正为“需求分析”。
        return R_SOURCE_REQUIREMENT

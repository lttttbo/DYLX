...
def target_value_for(case_kind: str, field_name: str, current_value: str) -> str:
    ...
    if case_kind == CASE_R and field_name == FIELD_SOURCE:
        ...
        if current_value in (
            ...
            R_SOURCE_NEW_EQUIVALENCE,
            R_SOURCE_EQUIVALENCE,
        ):
            return current_value

        # 原规则：精确的“需求分析、有效等价类”替换为组合后的规范名称。
        if current_value == R_SOURCE_OLD_EQUIVALENCE:
            return R_SOURCE_NEW_EQUIVALENCE

        # 新增规则：不包含“需求分析”，但包含“有效等价类”时，
        # 无论是否还包含其他来源内容，都统一修正为“等价类生成与分析”。
        if (
            R_SOURCE_REQUIREMENT not in current_value
            and R_SOURCE_EFFECTIVE_EQUIVALENCE in current_value
        ):

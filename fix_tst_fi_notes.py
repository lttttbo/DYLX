...
# -*- coding: utf-8 -*-
"""
检查并修正 .tst 文件中 TEST.NAME 以 .FI 结尾的测试用例说明：
  1. 用例类别：故障注入测试
  2. 用例来源：基于理论和经验的错误预测
...
TARGET_FIELDS: Sequence[Tuple[str, str]] = (
    ("用例类别", "故障注入测试"),
    ("用例来源", "基于理论和经验的错误预测"),
)
...
def is_note_boundary(line: str) -> bool:
    ...
    if stripped.startswith("TEST.") or stripped.startswith("--"):
        return True
    return bool(
        KNOWN_NOTE_FIELD_RE.match(body) or GENERIC_NOTE_FIELD_RE.match(body)
    )
    ...
    for case_start, case_end in case_blocks:
        ...
        if not case_name:
            result.findings.append(
                Finding(
                    ...
                    detail="测试用例块中 TEST.NAME 缺失或为空；无法判断是否为 .FI 用例。",
                )
            )
            continue

        if not case_name.upper().endswith(".FI"):
            continue

        result.fi_case_count += 1
        ...
        if notes_start is None or notes_end is None:
            result.findings.append(
                Finding(
                    ...
                    line=(name_line if name_line is not None else case_start) + 1,
                    case_name=case_name,
                    detail=".FI 用例缺少完整的 TEST.NOTES / TEST.END_NOTES 区域；未自动修改。",
                )
            )
            continue
        ...
        for field_name, target_value in TARGET_FIELDS:
            ...
            if not occurrences:
                ...
                insertions[notes_end].append(
                    f"{field_name}：{target_value}{eol}"
                )
                result.findings.append(
                    Finding(
                        kind="MISSING",
                ...
                for duplicate_index, _ in occurrences[1:]:
                    result.findings.append(
                        Finding(
                            ...
                            field_name=field_name,
                            target_value=target_value,
                            detail="同一 TEST.NOTES 区域存在重复字段；各处均会检查并统一。",
                        )
                    )
                    ...
                    if separate_value != target_value:
                        ...
                        result.findings.append(
                            Finding(
                                ...
                                target_value=target_value,
                                detail=f"字段名位于第 {field_index + 1} 行，值另起一行。",
                            )
                        )
                    continue
...
def update_summary(summary: Summary, result: FileResult) -> None:
    ...
    summary.fi_cases += result.fi_case_count
...
def build_report(
    ...
    report_lines.extend(
        [
            "",
            "汇总：",
            f"  扫描 .tst 文件：{summary.files_scanned}",
            f"  扫描 .FI 用例：{summary.fi_cases}",
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "递归检查 .tst 文件中 TEST.NAME 以 .FI 结尾的用例，"
            "统一用例类别和用例来源。默认仅检查。"
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--report",
        help="报告文件路径；默认写入检查目录下的 tst_fi_check_report.txt",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.report:
        ...
        report_path = report_base / "tst_fi_check_report.txt"
    ...
    print(f".FI 用例：{summary.fi_cases}")
    print(f"需要修改的文件：{summary.files_needing_changes}")
    print(f"实际写入的文件：{summary.files_written}")
    print(f"编辑数量：{summary.edits}")
    print(f"空值/缺失/结构异常：{summary.empty_values}/{summary.missing_fields}/{summary.structural_anomalies}")
